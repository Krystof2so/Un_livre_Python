...
\section{Le formatage des chaînes de caractères}
Pour illustrer diverses méthodes de formatage de chaînes de caractères, nous pouvons tester le script suivant:
\begin{lstlisting}
"""Voici divers exemples de formatage de 
chaînes de caractères."""
print("\nEXEMPLES DE FORMATAGE:\n" + "-" * 20)
# Accéder à un argument par sa position :
print('{0}, {1}, {2}'.format('un', 'deux', 'trois'))
# En sortie ==> 'un, deux, trois'
print('{}, {}, {}'.format('un', 'deux', 'trois'))
# En sortie ==> 'un, deux, trois'
print('{2}, {1}, {0}'.format('un', 'deux', 'trois'))
# En sortie ==> 'trois, deux, un'
liste_chiffres = ['un', 'deux', 'trois']
# Avec l'unpacking :
print('{2}, {1}, {0}'.format(*liste_chiffres)) 
# En sortie ==> 'trois, deux, un'
# Les indices peuvent être répétés :
print('{0}, {1}, {0}'.format('un', 'deux'))  
# En sortie ==> 'un, deux, un'

# Accéder à un argument par son nom :
print('Coordonnées: {latitude}, {longitude}'
      .format(latitude='37.24N', longitude='-115.81W'))
# En sortie ==> 'Coordonnées: 37.24N, -115.81W'
coord = {'latitude': '37.24N', 'longitude': '-115.81W'}
print('Coordonnées: {latitude}, {longitude}'.format(**coord))
# En sortie ==> 'Coordonnées: 37.24N, -115.81W'
...
print('{:<30}'.format('aligné à gauche'))
# En sortie ==> 'aligné à gauche               '
print('{:>30}'.format('aligné à droite'))
# En sortie ==> '               aligné à droite'
print('{:^30}'.format('centré'))
# En sortie ==> '            centré            '
# Avec '*' comme caractère de remplissage :
print('{:*^30}'.format('centré'))  
# En sortie ==> '************centré************' 

# Convertir une valeur dans différentes bases :
print("int: {0:d};  hex: {0:x};  oct: {0:o};  bin: {0:b}"
      .format(42))
# En sortie ==> 'int: 42;  hex: 2a;  oct: 52;  bin: 101010'
# Avec 0x, 0o ou 0b comme préfixes :
print("int: {0:d};  hex: {0:#x};  oct: {0:#o};  bin: {0:#b}"
      .format(42)) 
# En sortie ==> 'int: 42;  hex: 0x2a;  oct: 0o52;  
# bin: 0b101010'
octets = [192, 168, 0, 1]
print('{:02X}{:02X}{:02X}{:02X}'.format(*octets))
# En sortie ==> 'C0A80001'

# L'underscore pour séparer les milliers :
print('{:_}'.format(1234567890))
# En sortie ==> '1_234_567_890'

# Exprimer un pourcentage
points, total = 17, 24
print('Taux de réponses corrects: {:.2%}'
      .format(points/total))
# En sortie ==> 'Taux de réponses corrects: 70.83%'
\end{lstlisting}
\medskip

\section{Le module \texttt{string}}
Ce module fournit quelques constantes et permet de réaliser quelques opérations usuelles sur les chaînes de caractères\footnote{Documentation officielle : \url{https://docs.python.org/fr/3/library/string.html}}.
\medskip

\subsection*{Les constantes}
\begin{description}
	\item[\texttt{string.ascii\_lowercase}]: Une chaîne qui contient toutes les lettres de l'alphabet en minuscules, de \texttt{a} à \texttt{z}.
	\item[\texttt{string.ascii\_uppercase}]: Une chaîne qui contient toutes les lettres de l'alphabet en majuscules, de \texttt{A} à \texttt{Z}.
	\item[\texttt{string.ascii\_letters}]: Concaténation des deux précédentes constantes.
	\item[\texttt{string.digits}]: Contient tous les chiffres de \texttt{0} à \texttt{9}.
	\item[\texttt{string.hexdigits}]: Contient tous les caractères \texttt{ascii} hexadécimaux, soit \texttt{string. digits + 'abcdef' + 'ABCDEF'}\footnote{Ou bien : \texttt{string.digits + string.ascii\_lowercase[0:6] + string.ascii\_uppercase[0:6]}}.
	\item[\texttt{string.octdigits}]: \texttt{'01234567'} (ou bien \texttt{string.digits[0:8]}).
	\item[\texttt{string.punctuation}]: Contient tous les signes considérés comme des signes de ponctuation, soit:
	 \begin{verbatim}
		!"#$%&'()*+,-./:<?;=@>[\]^_`{|}~ 
	 \end{verbatim}
	\item[\texttt{string.whitespace}]: Contient tous les caractères considérés comme espaces, soit :
	 \begin{verbatim}
		' \t\n\r\v\f'
	 \end{verbatim}
	\item[\texttt{string.printable}]: Contient tous les caractères considérés comme affichables, soit : \texttt{string.digits + string.ascii\_letters + string.punctuation + string.whitespace}.
\end{description}
\medskip

\subsection*{La classe \texttt{Formatter}}
La classe \texttt{Formatter}\footnote{\url{https://docs.python.org/fr/3/library/string.html\#custom-string-formatting}} permet de créer et personnaliser des comportements de formatage en utilisant la même implémentation qu'avec la méthode primitive \texttt{format()} comme, par exemple, avec des méthodes de remplacement de champs spécifiques dans une chaîne. Cependant, pour un usage courant, la méthode \texttt{str.format()} est préférée pour sa simplicité et sa lisibilité. Mais la gestion des erreurs spécifiques au formatage est moins explicite par rapport à \texttt{Formatter}.
\medskip

\subsection*{Les chaînes modèles}
Le module \texttt{string} fournit également la classe \texttt{Template} qui implémente des règles de substitution de chaînes modèles. Les marqueurs de remplacement sont des variables précédées du signe \texttt{\$}. Ce mode de remplacement offre une meilleure sécurité que les méthodes de formatage conventionnelles. La syntaxe est à la fois claire et intuitive, et cela permet de créer facilement des modèles de chaînes réutilisables.
\medskip

Le constructeur, pour la classe ne prend qu'un seul argument qui est la chaîne modèle\footnote{Cette phrase prendra son sens quand sera abordée la programmation orientée objet}. Illustrons tout simplement cela par le script qui suit:
\begin{lstlisting}
from string import Template

print("\nAVEC LA CLASSE 'Template' DU MODULE 'string' :\n" 
      + "-" * 20)
# Méthode '.sustitute()'
qui_ecrit_quoi = Template(
    "$auteur a écrit l'ouvrage '$livre'")
print(qui_ecrit_quoi.substitute(auteur="Philippe DJIAN", 
                                livre="Maudit Manège"))
print(qui_ecrit_quoi.substitute(
    auteur="Ernest HEMINGWAY",
    livre="Pour qui sonne le glas"))
"""En sortie ==>
Philippe DJIAN a écrit l'ouvrage 'Maudit Manège'
Ernest HEMINGWAY a écrit l'ouvrage 
...
for auteur, ouvrage in LIVRES.items():
    print(Template("$auteur a écrit '$ouvrage'")
          .substitute(auteur=auteur, ouvrage=ouvrage))
"""En sortie ==>
DJIAN a écrit 'Sotos'
KEROUAC a écrit 'Sur la route'
HEMINGWAY a écrit 'Paris est une fête'"""

# Méthode safe_substitute() :
qui_ecrit_quoi = Template(
    "$auteur a écrit l'ouvrage '$livre'")
print(qui_ecrit_quoi.safe_substitute(
    auteur="Philippe DJIAN"))
# Alors que : 'print(qui_ecrit_quoi.substitute(
# auteur="Philippe DJIAN"))' provoque une 'KeyError'

# Méthode get_identifiers() :
print(qui_ecrit_quoi.get_identifiers()) 
# Retourne ube liste des identifiants valides.
# En sortie ==> ['auteur', 'livre']
\end{lstlisting}
\medskip
